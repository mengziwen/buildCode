    ...
    <div class="field">
      <div class="title">
        ...
        <div class="searchInput">
          <Input v-model="value" placeholder="请输入岗位名称" style="width: 120px"></Input>
        </div>
        <div class="searchBtn">
      ...
      <Modal
        ...
        <p slot="header">
          <span v-if="add">添加岗位</span>
          <span v-if="!add">编辑岗位</span>
          ...
          <Form ref="formData" :model="formData" :rules="ruleValidate" :label-width="80">
            <FormItem label="岗位名称" prop="name">
              <Input v-model="formData.name" placeholder="请输入岗位名称"></Input>
            </FormItem>
          </Form>
        <div slot="footer">
    ...
    data () {
              ...
              return h('div', [
                h('Button', {
                  ...
                  style: {
                    ...
                    on:{
                        'click':()=>{
                            this.editStation(params.index,params.row.stationId);
                        }
                }
                }, '编辑'),
                h('Button', {
                  ...
                  style: {
                    ...
                    on:{
                        'click':()=>{
                            this.deleteStation(params.row.stationId);
                        }
                    }
                }, '删除')
              ]);
      ...
      methods:{
          init(){
              ...
              this_obj.data = [];
              this_obj.pages = 0;
              var url = '/auth/stations/pageNum/'+this_obj.currentPage+'/pageSize/10';
              this.$http.get(url).then(res => {
                  this.table_isLoading = false;
                  this_obj.pages = Math.ceil(res.data.obj.sum/10);
                  this_obj.data = res.data.obj.stationInfos;
              });
          },
          handleSubmit (name) {
              var this_obj = this;
              this.$refs[name].validate((valid) => {
                  if (valid) {
                      ...
                      if(!this.add){
                          var url = '/auth/stations/'+this_obj.formData.id;
          ...
          editStation:function(index,param){
              ...
              this.add= false;
              this.modal1 = true;
              this.formData.name = this.data[index].stationName;
              this.formData.id = param;
          },
          deleteStation:function(param){
              var this_obj = this;
              var url = '/auth/stations/'+param;
          ...
          searchStations:function(){
              ...
              var url = '/auth/stations/station/'+keywords;
              if(keywords == ''){
                  this.init();
              }else{
                  this.$http.get(url).then(res=>{
